    ...
    try:
        ...
        if to_show == 'options':
            ...
            for i in config.listener_database:
                tmp_list.append([i,config.listener_database[i][2],config.listener_database[i][0] + ':' + str(config.listener_database[i][1])])
            tmp_list.sort()
            for i in tmp_list:
            ...
            print '[-]Please specify a valid argument, ["options"|"listeners"]'
    except IndexError:
        print '[-]Please specify what to show, ["options"|"listeners"]'